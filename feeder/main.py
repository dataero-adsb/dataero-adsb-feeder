...
# Feed mode:
#   reduce : readsb forwards a *reduced* Beast stream (with UUID) to the Dataero
#            aggregator natively via its beast_reduce_plus_out connector
#            (configured by feeder/configure_readsb_reduce.sh). This process does
#            NOT POST; its only job is the heartbeat. The UUID is carried in-band,
#            so provenance doesn't depend on the public-IP binding (the heartbeat
#            is kept for liveness).
#   json   : POST aircraft.json over HTTPS every 2s plus the heartbeat. Works
#            with any decoder.
